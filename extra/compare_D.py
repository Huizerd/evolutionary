...
def compare_D(sim_folder, real_folder):
    ...
    # Create plot
    fig, axs = plt.subplots(6, 2, sharex=True, figsize=(10, 10), dpi=200)
    axs[0, 0].set_title("sim")
    axs[0, 1].set_title("real")
    axs[0, 0].set_ylabel("height [m]")
    axs[1, 0].set_ylabel("velocity [m/s]")
    axs[2, 0].set_ylabel("thrust [g]")
    axs[3, 0].set_ylabel("divergence [1/s]")
    axs[4, 0].set_ylabel("divergence dot [1/s2]")
    axs[5, 0].set_ylabel("spikes [?]")
    ...
    for i, f in enumerate(sim_files):
        ...
        # Thrust
        axs[2, 0].plot(data["time"], data["tsp"], label=f"run {i}")
        # Divergence
        axs[3, 0].plot(data["time"], data["div"], label=f"run {i}")
        # Divergence dot
        axs[4, 0].plot(data["time"], data["divdot"], label=f"run {i}")
        # Spikes
        axs[5, 0].plot(
    ...
    for i, f in enumerate(real_files):
        ...
        # Thrust
        axs[2, 1].plot(data["time"], data["thrust"], label=f"run {i}")
        # Divergence
        axs[3, 1].plot(data["time"], data["div"], label=f"run {i}")
        # Divergence dot
        axs[4, 1].plot(data["time"], data["divdot"], label=f"run {i}")
        # Spikes
        axs[5, 1].plot(
            data["time"], data["spike_count"] / data["time"], label=f"run {i}",
        )
    ...
    # Set limits
    for ax, ylim in zip(
        axs, [[0, 4], [-1.0, 0.5], [-0.1, 0.1], [-1, 2], [-40, 40], [0, 1000]]
    ):
        for a in ax:
            a.set_ylim(ylim)
    fig.tight_layout()
