...
def vis_distributions(config, parameters, debug=False, no_plot=False):
    # Get files
    if len(glob.glob(parameters)) > 1:
        files = glob.glob(parameters)
    else:
        raise ValueError("Include more than one network file for analysis")

    # Get empty network, and fill with first set of parameters (needed for init)
    network = build_network(config)
    network.load_state_dict(torch.load(files[0]))
